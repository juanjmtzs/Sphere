...
public class UnicastBMVYesBytes {
    ...
    public UnicastBMVYesBytes(String username, String password, String group, String host, int port, Decoder.Potocol protocol, String receiverIP) throws IOException {
        Username = username;
        Password = password;
        Group = group;
    ...
    private void loop() throws IOException {
        ...
        while (connected == true && clientSocket.isConnected() == true) {
            ...
            MDBMV.Update.setText(""
                    ...
                    + "Total Messages:  " + new Integer((Decoder.BMVYesBytes.count_depth
                            + Decoder.BMVYesBytes.count_probable_allocation_price
                            + Decoder.BMVYesBytes.count_continuos_auction_beginning
                            + Decoder.BMVYesBytes.count_status_changes
                            + Decoder.BMVYesBytes.count_to_middle_price_existing_quotes
                            + Decoder.BMVYesBytes.count_local_and_global_stock_markets
                            + Decoder.BMVYesBytes.count_debt_metals_and_money_market
                            + Decoder.BMVYesBytes.count_capital_market_warrants
                            + Decoder.BMVYesBytes.count_capital_market_TRACS
                            + Decoder.BMVYesBytes.count_mutual_funds
                            + Decoder.BMVYesBytes.count_underlying_value_on_warrants
                            + Decoder.BMVYesBytes.count_tradability
                            + Decoder.BMVYesBytes.count_trade_cancellation
                            + Decoder.BMVYesBytes.count_weighted_average_price_settlement_prices
                            + Decoder.BMVYesBytes.count_best_offer
                            + Decoder.BMVYesBytes.count_capital_trades
                            + Decoder.BMVYesBytes.count_event_systems
                            + Decoder.BMVYesBytes.count_virtual_trades
                            + Decoder.BMVYesBytes.count_mutual_fund_trades
                            + Decoder.BMVYesBytes.count_registry_operations
                            + Decoder.BMVYesBytes.count_derivatives_market_trades
                            + Decoder.BMVYesBytes.count_order_addition
                            + Decoder.BMVYesBytes.count_order_change
                            + Decoder.BMVYesBytes.count_execution_of_orders
                            + Decoder.BMVYesBytes.count_volume_update
                            + Decoder.BMVYesBytes.count_orders_cancellation
                            + Decoder.BMVYesBytes.count_instruments_statistics
                            + Decoder.BMVYesBytes.count_inavs
                            + Decoder.BMVYesBytes.count_general_indexes
                            + Decoder.BMVYesBytes.count_indexes_samples
                            + Decoder.BMVYesBytes.count_open_interest
                            + Decoder.BMVYesBytes.count_public_offerings
                            + Decoder.BMVYesBytes.count_futures_operations_and_swaps_derivatives_market
                            + Decoder.BMVYesBytes.count_derivatives_market_strategies
                            + Decoder.BMVYesBytes.count_dollar_buy_sell
                            + Decoder.BMVYesBytes.count_short_sales_balances_per_instrument
                            + Decoder.BMVYesBytes.count_capital_markets_multiples
                            + Decoder.BMVYesBytes.count_benchmarks
                            + Decoder.BMVYesBytes.count_capitalization_rules
                            + Decoder.BMVYesBytes.count_origin_closing_price
                            + Decoder.BMVYesBytes.count_consolidated_relation_between_bmvbiva_instruments
                            + Decoder.BMVYesBytes.count_consolidated_system_events
                            + Decoder.BMVYesBytes.count_consolidated_capitals_market_trades
                            + Decoder.BMVYesBytes.count_consolidated_trades_cancellation
                            + Decoder.BMVYesBytes.count_consolidated_best_bid
                            + Decoder.BMVYesBytes.count_consolidated_registration_of_order
                            + Decoder.BMVYesBytes.count_consolidated_execution_of_orders
                            + Decoder.BMVYesBytes.count_consolidated_cancellation_of_orders
                            + Decoder.BMVYesBytes.count_consolidated_amendment_of_orders
                            + Decoder.BMVYesBytes.count_consolidated_updating_of_volume
                            + Decoder.BMVYesBytes.count_consolidated_changes_in_status
                            + Decoder.BMVYesBytes.count_consolidated_reference_price
                            + Decoder.BMVYesBytes.count_consolidated_probable_price_of_allotment
                            + Decoder.BMVYesBytes.count_consolidated_startup_of_ongoing_bidding_sessions
                            + Decoder.BMVYesBytes.count_consolidated_existence_of_bids_at_mean_price
                            + Decoder.BMVYesBytes.count_consolidated_weighted_average_price_ppp
                            + Decoder.BMVYesBytes.count_consolidated_investment_companies_trades
                            + Decoder.BMVYesBytes.count_consolidated_inavs
                            + Decoder.BMVYesBytes.count_consolidated_probable_price_of_allotment_volume_indicator
                            + Decoder.BMVYesBytes.count_consolidated_spread_quality
                            + Decoder.BMVYesBytes.count_consolidated_message_ratio
                            + Decoder.BMVYesBytes.count_consolidated_depth_at_best_prices
                            + Decoder.BMVYesBytes.count_consolidated_effective_spread
                            + Decoder.BMVYesBytes.count_consolidated_trading_sniper
                            + Decoder.BMVYesBytes.count_consolidated_quotes_quality
                            + Decoder.BMVYesBytes.count_consolidated_price_leaderboard
                            + Decoder.BMVYesBytes.count_consolidated_spread
                            + Decoder.BMVYesBytes.count_consolidated_benchmark_trade
                            + Decoder.BMVYesBytes.count_consolidated_hour_tracker
                            + Decoder.BMVYesBytes.count_consolidated_big_picture
                            + Decoder.BMVYesBytes.count_consolidated_local_and_global_stock_market
                            + Decoder.BMVYesBytes.count_consolidated_capital_market_warrants
                            + Decoder.BMVYesBytes.count_consolidated_capital_market_tracs
                            + Decoder.BMVYesBytes.count_consolidated_mutual_funds
                            + Decoder.BMVYesBytes.count_consolidated_debt_and_metals_market
                            + Decoder.BMVYesBytes.count_global_and_local_catalog
                            + Decoder.BMVYesBytes.count_turn_over_ratio_per_security
                            + Decoder.BMVYesBytes.count_securities_suspension
                            + Decoder.BMVYesBytes.count_securities_unsuspension
                            + Decoder.BMVYesBytes.count_negotiation_state_change_SUBRM
                            + Decoder.BMVYesBytes.count_official_closing_price)) + "\n"
                    ...
                    + "     Origin Closing Price (~):  " + Decoder.BMVYesBytes.count_origin_closing_price + "\n"
                    + "     Relation between BMV/BIVA Instruments(j):  " + Decoder.BMVYesBytes.count_consolidated_relation_between_bmvbiva_instruments + "\n"
                    + "     System Events(7):  " + Decoder.BMVYesBytes.count_consolidated_system_events + "\n"
                    + "     Capitals Market Trades (p):  " + Decoder.BMVYesBytes.count_consolidated_capitals_market_trades + "\n"
                    + "     Trades Cancellation (q):  " + Decoder.BMVYesBytes.count_consolidated_trades_cancellation + "\n"
                    + "     Best Bid(m):  " + Decoder.BMVYesBytes.count_consolidated_best_bid + "\n"
                    + "     Registration of Order(n):  " + Decoder.BMVYesBytes.count_consolidated_registration_of_order + "\n"
                    + "     Execution of Orders(k):  " + Decoder.BMVYesBytes.count_consolidated_execution_of_orders + "\n"
                    + "     Cancellation of Orders(u):  " + Decoder.BMVYesBytes.count_consolidated_cancellation_of_orders + "\n"
                    + "     Amendment of Orders(v):  " + Decoder.BMVYesBytes.count_consolidated_amendment_of_orders + "\n"
                    + "     Updating of Volume(w):  " + Decoder.BMVYesBytes.count_consolidated_updating_of_volume + "\n"
                    + "     Changes in Status(9):  " + Decoder.BMVYesBytes.count_consolidated_changes_in_status + "\n"
                    + "     Reference Price(8):  " + Decoder.BMVYesBytes.count_consolidated_reference_price + "\n"
                    + "     Probable Price of Allotment(i):  " + Decoder.BMVYesBytes.count_consolidated_probable_price_of_allotment + "\n"
                    + "     Startup of Ongoing Bidding Sessions()):  " + Decoder.BMVYesBytes.count_consolidated_startup_of_ongoing_bidding_sessions + "\n"
                    + "     Existence of Bids At Mean Price(,):  " + Decoder.BMVYesBytes.count_consolidated_existence_of_bids_at_mean_price + "\n"
                    + "     Weighted Average Price (PPP)(6):  " + Decoder.BMVYesBytes.count_consolidated_weighted_average_price_ppp + "\n"
                    + "     Investment Companies Trades(():  " + Decoder.BMVYesBytes.count_consolidated_investment_companies_trades + "\n"
                    + "     INAV's(]):  " + Decoder.BMVYesBytes.count_consolidated_inavs + "\n"
                    + "     Probable Price of Allotment/Volume Indicator(\\):  " + Decoder.BMVYesBytes.count_consolidated_probable_price_of_allotment_volume_indicator + "\n"
                    + "     Spread Quality({):  " + Decoder.BMVYesBytes.count_consolidated_spread_quality + "\n"
                    + "     Message Ratio(}):  " + Decoder.BMVYesBytes.count_consolidated_message_ratio + "\n"
                    + "     Depth at Best Prices(^):  " + Decoder.BMVYesBytes.count_consolidated_depth_at_best_prices + "\n"
                    + "     Effective Spread(=):  " + Decoder.BMVYesBytes.count_consolidated_effective_spread + "\n"
                    + "     Trading Sniper(<):  " + Decoder.BMVYesBytes.count_consolidated_trading_sniper + "\n"
                    + "     Quotes Quality(|):  " + Decoder.BMVYesBytes.count_consolidated_quotes_quality + "\n"
                    + "     Price Leaderboard(@):  " + Decoder.BMVYesBytes.count_consolidated_price_leaderboard + "\n"
                    + "     Spread(;):  " + Decoder.BMVYesBytes.count_consolidated_spread + "\n"
                    + "     Benchmark Trade(:):  " + Decoder.BMVYesBytes.count_consolidated_benchmark_trade + "\n"
                    + "     Hour Tracker(/):  " + Decoder.BMVYesBytes.count_consolidated_hour_tracker + "\n"
                    + "     Big Picture('):  " + Decoder.BMVYesBytes.count_consolidated_big_picture + "\n"
                    + "     Local and Global Stock Market(h):  " + Decoder.BMVYesBytes.count_consolidated_local_and_global_stock_market + "\n"
                    + "     Capital Market Warrants(T):  " + Decoder.BMVYesBytes.count_consolidated_capital_market_warrants + "\n"
                    + "     Capital Market TRACS([):  " + Decoder.BMVYesBytes.count_consolidated_capital_market_tracs + "\n"
                    + "     Mutual Funds(0):  " + Decoder.BMVYesBytes.count_consolidated_mutual_funds + "\n"
                    + "     Debt and Metals Market(.):  " + Decoder.BMVYesBytes.count_consolidated_debt_and_metals_market + "\n"
                    ...
                    + "     Official Closing Price (N):  " + Decoder.BMVYesBytes.count_official_closing_price
                    + "\n");

            System.out.println("[TCP-IN] Packet:" + Arrays.toString(bb.array()));
            System.out.println("[TCP-IN] Lenght:" + Arrays.toString(len) + " Decoded:" + new BigInteger(len));
            System.out.println("[TCP-IN] Messages In Package:" + Arrays.toString(messagesInPackage) + " Decoded:" + new BigInteger(messagesInPackage));
            System.out.println("[TCP-IN] Group:" + Arrays.toString(grupo) + " Decoded:" + new BigInteger(grupo));
            System.out.println("[TCP-IN] Session:" + Arrays.toString(session) + " Decoded:" + new BigInteger(session));
            System.out.println("[TCP-IN] Sequence:" + Arrays.toString(sequence) + " Decoded:" + new BigInteger(sequence));
            System.out.println("[TCP-IN] Timestamp:" + Arrays.toString(timestamp) + " Decoded:" + new BigInteger(timestamp));
            System.out.println("[TCP-IN] Messages:" + Arrays.toString(messages));
            ...
            switch (type) {
                ...
                case PacketType.BMV_login_response_type:
                    System.out.print(""
                            + "[BMV-IN]{"
                            + "\"Message\":{"
                            + "\"Bytes\":" + "\"" + Arrays.toString(messages).replace("[", "").replace(",", "").replace("]", "") + "\"" + ","
                            + "\"Name\":" + "\"" + "Login Response" + "\"" + ","
                            + "\"Type\":" + "\"" + "&" + "\"" + ","
        ...
        if (clientSocket != null) {
            ...
            if (clientSocket.isConnected() == true) {
                ...
                try {
                    ...
                    System.out.print("\"Bytes\":" + "\"" + Arrays.toString(ByteBufferAux.array()).replace("[", "").replace(",", "").replace("]", "") + "\"" + ",");
                    ...
                    if (MDBMV.viewMessages.isSelected()) {
                        MDBMV.logsArea.append(""
                                + "[BMV-OUT]{"
                                + "\"Message\":{"
                                + "\"Bytes\":" + "\"" + Arrays.toString(ByteBufferAux.array()).replace("[", "").replace(",", "").replace("]", "") + "\"" + ","
                                + "\"Name\":" + "\"" + "Login Request" + "\"" + ","
                                + "\"Type\":" + "\"" + "!" + "\"" + ","
                                + "\"Group\":" + group + ","
                                + "\"User\":" + "\"" + user + "\""
    ...
    public static void StartHandling(String[] args) throws IOException, UnsupportedLookAndFeelException {
        ...
        try {
            ...
            final UnicastBMVYesBytes s = new UnicastBMVYesBytes(args[3], args[4], args[5], args[0], Integer.parseInt(args[1]), new Decoder.BMVYesBytes(), args[6]);
